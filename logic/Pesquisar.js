...
class Pesquisar {
	...
	maisPopulares(idGenero) {
		var xhttp = new XMLHttpRequest();
		var token = new GerenciarToken();

		// manipula a URL da API adicionando o token de acesso e o ID de gênero a ser pesquisado
		var url = "http://api.themoviedb.org/3/discover/movie?api_key=" + token.geraNovo() + "&sort_by=popularity.desc&with_genres=" + idGenero;
		var resultadoPesquisa = new Array();
		xhttp.open("GET", url, false);
		xhttp.send();
